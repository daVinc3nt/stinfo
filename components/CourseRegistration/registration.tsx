...
import { ClassOperation, token } from "@/ambLib/amb";
import { CourseOperation, UpdatingCourseInfo } from "@/ambLib/amb";
...
export default function Registration(props: { token: token }) {
    ...
    const [registerServer, setRegisterSever] = useState<RegisterClassInfo[]>([])
            ...
            await course.findAllCourses(temp, props.token).then(data => {
                if (data.data) {
                    ...
                    for (let i = 0; i < x.length; i++) {
                        lst.push({ class_id: [], course_id: x[i].course_id, course_name: x[i].course_name, credits: x[i].credits, course_type: x[i].course_type })
                    }
                    setCourselst(lst)
                }
            })
    ...
    const handleRegister = (id: number, class_id: string) => {
        const temp = chooseClass
        temp[id] = class_id
        console.log(temp)
        setChooseClass(temp)
    }
    const handleDelt = (idx) => {
        ...
        if (check) {
            ...
            const temp1 = [];
            delt.map((item, index) => {
                if (delt[index] == 1) { temp.push(choose[index]); temp1.push(1); }
            })
            setDelt(temp1);
            setChoose(temp);
            setCheck(1);
            ...
            <div className="flex-1 flex flex-col lg:ml-6 ">
                                    ...
                                    <div className="flex flex-col bg-white">
                                        {course[curcourse].class_id.length != 0 &&
                                            course[curcourse].class_id.map((item) => {
                                                return (
                                                    <button className="p-1 focus:bg-blue-500 hover:bg-blue-500 text-sm" onClick={() => { handleRegister(curcourse, item) }}>{item}</button>
                                                )
                                            })
                                        }
                                    </div>
                ...
                <div className=" p-2 border rounded-lg bg-green-300">
                    ...
                    <div className="flex flex-row justify-between items-center mb-4">
                        ...
                        <button className="bg-green-500 p-1 text-white rounded-lg shadow-lg hover:text-black hover:scale-110">Xác nhận đăng ký</button>
                    </div>
                    <div className="flex flex-col h-screen_1/5">

                        <div className="grid grid-cols-3 gap-3 overflow-auto">
                            {
                                choose.map((item, index) => {
                                    if (chooseClass[index] != "")
                                        return (
