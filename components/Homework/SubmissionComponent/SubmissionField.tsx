...
const SubmissionField = (props: { role: string }) => {
	...
	let myClassID, myCourseName;

	

	if (typeof window !== 'undefined') {
		myClassID = window?.localStorage.getItem("currentClassID");
		console.log("myClassID: ", myClassID);
		myCourseName = window?.localStorage.getItem("currentCourseName");
	}

	const [classID, setClassID] = useState<ClassID>({ class_id: myClassID });
	...
	// useEffect(() => {
	// 	const newToken = { token: Cookies.get("token") };
	...
	console.log("classID HERE: ", classID);
	...
	useEffect(() => {
		...
		if (userToken && classID.class_id) {
			console.log("run something here!!!!!!!!!!!!!!!");
			operation.showSubmitFile({ class_id: String(classID.class_id.toString()) }, userToken)
				.then(data => {
					const getData = data;
					if (getData.data) {
			...
			<div className="py-3">
				...
				<input type='file' id="selected_file" className=' hidden' 
					...
					if (newFile) {
						...
						console.log("token from uploading: ", userToken);
						console.log("classID from uploading: ", classID);
						operation.submitFile(newFile, { class_id: String(classID.class_id) }, userToken)
							.then(response => {
						...
						operation.showSubmitFile({ class_id: String(classID.class_id) }, userToken)
							.then(data => {
								const getData = data;
								if (getData.data) {
									// console.log("list of submit File: ", getData);
									setListFileName(getData.data);
									setNumOfFile(getData.data.length);
									console.log("showSubmitFile run from UPLOADING BUTTON");
								}
							})
					}
				}}
	...
	const DeleteFileButton = (props: { deletedFileName: string }) => {
		...
		const handleDelete = () => {
			...
			operation.deleteSubmitFile({ class_id: String(classID.class_id), filename: props.deletedFileName }, userToken)
				.then(response => {
...
const ReviewField = () => {
	...
	if (typeof window !== 'undefined') {
		teacherName = window?.localStorage.getItem("currentTeacher");
		score = window?.localStorage.getItem("currentScore");
	}
